...
def cluster_routes (train_df,test_df,num_cluster=500):
    ...
    db =  MiniBatchKMeans(init='k-means++', n_clusters=num_cluster, batch_size=10*6,n_init=10, init_size=3*num_cluster,max_no_improvement=400, verbose=0,random_state=0).fit(concat)
    labels = db.labels_
    ...
    db =  MiniBatchKMeans(init='k-means++', n_clusters=100, batch_size=10*6,n_init=10,init_size=3*100, max_no_improvement=400, verbose=0,random_state=0).fit(concat)
    labels = db.labels_
...
def read_csv_sampled(path, frac, chunksize=10**5, random_state=None,train=True):
    ...
    for df_chunk in tqdm(pd.read_csv(path, chunksize=chunksize,nrows=30000000)):
        if train:
...
def modelling(train,y,num_splits=5):
    ...
    models = {}
    # Fit for every cluster
    modelstart = time.time()
    for trn_idx in tqdm(train['cluster'].unique()):
        indx = train.loc[train['cluster']==trn_idx,:].index
		dtrain = lgb.Dataset(train.loc[indx,:].drop('cluster',axis=1), label=y.loc[indx], free_raw_data=False)
        clf = lgb.train(
            params=lgbm_params,
            train_set=dtrain,
        ...
        models[trn_idx] = clf
        oof_preds = clf.predict(train.loc[indx,:].drop('cluster',axis=1))
        print("Cluster : {0} - RMSE : {1}".format(trn_idx,mean_squared_error(y.loc[indx], oof_preds) ** .5))
...
def main():
    ...
    train, test = add_holidays(train,test)
    ...
    fold_preds_exp = pd.Series()#np.zeros(test.shape[0])
	KID = pd.Series()
	for clust in tqdm(test['cluster'].unique()):
		indx = test.loc[test['cluster']==clust].index
		KID = pd.concat([KID,test.loc[indx,'key']])
		fold_preds_exp = pd.concat([fold_preds_exp,models[clust].predict(test.loc[indx,:]).drop(['key','cluster'],axis=1)])
